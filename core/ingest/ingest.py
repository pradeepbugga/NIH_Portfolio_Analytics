...
def ingest_year(
    year: int, conn, cur, org_cache: dict, ingest_id: str, POLICY: dict, metrics: dict
):
    """
        ...
        while offset < total:
                ...
                try:
                    ...
                    if attempt == MAX_RETRIES:
                        raise RuntimeError(
                            ...
                            f"(FY={year}, agency={agency}, offset={offset})"
                        )
            ...
            for result in data.get("results", []):
                try:
                    ...
                    conn.rollback()
                    grant_id = (
                        f"{result.get('project_num')}-{result.get('subproject_id')}"
                        if result.get("subproject_id")
                        else result.get("project_num")
                    )
