...
from sentence_transformers.cross_encoder.evaluation import CrossEncoderCorrelationEvaluator, CrossEncoderRerankingEvaluator

MODEL_VERSION_NUMBER = 'v3'
NUM_EPOCHS = 2 # we found 2 epochs to be sufficient for convergence on this dataset
EVALUATION_STEPS = 36 # for batch size 4 and 135 train labels, we get almost 34 steps per epoch
...
df["disease_label"] = pd.to_numeric(
    df["disease_label"],
    errors="coerce"
)
...
examples = [
    InputExample(
        texts=[row.query, row.doc_text],
        label=float(row.disease_label)
    )
...
sentence_pairs = [
    [ex.texts[0], ex.texts[1]]   # [query, doc]
    for ex in val_ex
]

scores = [float(ex.label) for ex in val_ex]
assert all(isinstance(s, float) for s in scores), f"All scores must be float type, but got: {[type(s) for s in scores]}"
