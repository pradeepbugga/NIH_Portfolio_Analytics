...
FUNCTION
 Include all categories with function_scores ≥ 1.

 FUNCTION should not be empty unless:
 - the project is purely mechanistic with no measurable system, OR
 - the project is education-only

 Maximum: 2 functions

 If more than 2 functions:
 keep highest scores using priority:
 Infrastructure > Diagnostic > Therapeutic > Tool
...
FUNCTION
Research Tool / Method → any method, assay, model, or system used or developed in the project
...
the intervention is evaluated as a treatment or prevention strategy, AND
success is defined by improving a clinical or disease outcome

Do NOT assign Therapeutic if:
the intervention is used to probe or perturb a biological system, OR
the goal is to understand mechanism, even if outcomes are measured
...
Research Tool Function Rule:
If intent_scores["Research Tool / Method"] ≥ 1:
→ function_scores["Research Tool / Method"] must be ≥ 1

This rule always applies, regardless of whether the tool is a primary output.

Clinical Function Rule:
If intent_scores["Clinical / Health Systems"] = 2 AND no other function ≥1:
→ function_scores["Diagnostic / Biomarker"] = 1

Speculative statements
 “Potential application” ≠ assign label
 Only assign if actually developed, tested, or applied

MECHANISTIC FUNCTION RULE

If intent_scores["Mechanistic / Basic Science"] = 2
AND all function_scores = 0:
→ set function_scores["Research Tool / Method"] = 1


EDUCATION vs INFRASTRUCTURE RULE

Do NOT assign Research Infrastructure / Programmatic if:

the project is primarily a training or education program
even if it involves structured programs, curricula, or institutional coordination

Infrastructure refers ONLY to:
...
When assigning INTENT:
prioritize who/what is being studied over how it is described

Specifically:
studies of patients, outcomes, or symptoms → Clinical
even if phrased as “understanding relationships”


INTENT CONSISTENCY RULE

Any category with intent_score = 2 must appear in the final "intent" list.
...
FINAL TASK
Return:
{   "intent": [],   "function": [],  "intent_scores": {},   "function_scores": {} }
    ...
    async with SEM:
        try:
            response = await with_retry(
                ...
                safety_identifier="C-RoKsn42ipaPa"
            )
            ...
            if cached_tokens == 0:
                CACHE_MISS_STREAK += 1
                print(f"Cache miss for grant {grant_id}. Current streak: {CACHE_MISS_STREAK}")
            else:
                CACHE_MISS_STREAK = 0

            if CACHE_MISS_STREAK >= MAX_ALLOWED_MISSES:
                print(f"Warning: Cache miss streak of {CACHE_MISS_STREAK}. Consider pausing or investigating.")
                raise Exception("CacheNotStickingError: Multiple consecutive cache misses.")


            content = response.output_text
            
            log = usage.model_dump()  # Capture the log for debugging
            ...
            if "CacheNotStickingError" in str(e):
                raise e
...
async def main():
    ...
    # load from jsonl
    jsonl_path = "./gpt_dual_results_run1.jsonl"
    with open(jsonl_path, "r") as f:
        existing_results = [json.loads(line) for line in f]

    # take first 50 grant ids from jsonl
    existing_grant_ids = set(result["grant_id"] for result in existing_results[:50])

        
    # 2. Create list of tuples
    grants = list(zip(df["grant_id"], df["title"], df["abstract"]))
   
    # now filter grants to include only those that are in existing_grant_ids
    grant_sample = [grant for grant in grants if grant[0] in existing_grant_ids]
    ...
    # prime initial run to initate caching
    first_id, first_title, first_abstract = grant_sample[0]

    prime_result = await classify_grant(first_id, first_title, first_abstract)

    with open("gpt_dual_results.jsonl", "w") as f:
        f.write(json.dumps(prime_result) + "\n")
    
    pbar.update(1)

    remaining_grants = grant_sample[1:]

    await asyncio.sleep(5)  # brief pause to allow cache to populate after priming

    # Use 'a' (append) or 'w' (write) mode
    with open("gpt_dual_results.jsonl", "a") as f:
        # Start workers
        workers = [asyncio.create_task(worker(queue, f, pbar)) for _ in range(NUM_WORKERS)]

        # Start producer - Changed 'cur' to 'grants'
        producer_task = asyncio.create_task(producer(queue, remaining_grants))
